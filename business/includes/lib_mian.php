...
function get_order_list($is_page=true,$action=null){
	...
	$sql = "SELECT o.order_id,o.user_id, o.order_sn,o.mobile,o.msg_code, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
...
//echo $sql;exit;
	...
	foreach ($row AS $key => $value)
	{
		...
		$row[$key]['user_name']=$GLOBALS['db']->getOne('select user_name from hhs_users where user_id='.$value['user_id']);
	}
...
function get_sale_list($is_pagination = true,$suppliers_id){
    ...
    $where = " WHERE og.order_id = oi.order_id". order_query_sql('finished', 'oi.') .
             " AND oi.suppliers_id='$suppliers_id'  and oi.add_time >= '".$filter['start_date']."' AND oi.add_time < '" . ($filter['end_date'] + 86400) . "'";
...
function get_type_list($supp_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
	  ...
	  $arr['act']='bunus';
	  $pager  = get_pager('suppliers.php', $arr, $record_count, $page);	
      $sql = "SELECT * FROM " .$GLOBALS['hhs']->table('bonus_type'). " where suppliers_id='$supp_id' ORDER BY $filter[sort_by] $filter[sort_order] limit 0,10";
    $arr = array();
    $res = $GLOBALS['db']->query($sql);
...
function get_goods_list($filter)
{
	...
	if($filter->cat_id>0)
	{
		$where.=" and my_cat_id=".$filter->cat_id;
	}
