...

{
	...
	getUrl('suppliers.php?act=my_goods');
...
elseif($action  =='my_goods_batch'){
    ...
    if (!empty($_POST['checkbox']))
    {
        ...
        elseif(isset($_POST['up_sale'])){
            $sql = "update " . $hhs->table('goods'). " set is_on_sale=1 WHERE goods_id " . db_create_in($goods_id);
            $db->query($sql);
        }
        elseif(isset($_POST['down_sale'])){
            $sql = "update " . $hhs->table('goods'). " set is_on_sale=0 WHERE goods_id " . db_create_in($goods_id);
            $db->query($sql);
        }
...
{    
	...
	$sql = "UPDATE " .$hhs->table('suppliers'). " SET business_scope = '".$business_scope."',email1='".$email1."', phone1='".$phone1."', url_name='".$url_name."',province_id = '".$province_id."', city_id = '".$city_id."',district_id = '".$district_id."',address = '".$address."',email = '".$email."',phone = '".$phone."',  shopowner_phone = '".$shopowner_phone."' ,map_info='$map_info',suppliers_desc = '".$suppliers_desc."',qq = '".$qq."',real_name = '".$real_name."',identification_card = '".$identification_card."',company_name = '".$company_name."',business_license_number = '".$business_license_number."'".$where." WHERE suppliers_id = '".$suppliers_id."'";
	$res = $db->query($sql);
	if($res){
...
function account_detail_list(){
    ...
    $sql = "SELECT sat.*,(sat.amount-sat.commission) as money,o.suppliers_id,o.consignee,o.pay_name,o.user_id " .
    ...
    return array('row'=>$row,'total_amount'=>$total_amount,'total_commission'=>$total_commission,'total_money'=>$total_money);
