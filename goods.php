...
if (!$smarty->is_cached('goods.dwt', $cache_id))
{
    ...
    else
    {
        ...
        assign_template('c', $catlist);
         ...
         /* 上一个商品下一个商品 */
        $prev_gid = $db->getOne("SELECT goods_id FROM " .$hhs->table('goods'). " WHERE cat_id=" . $goods['cat_id'] . " AND goods_id > " . $goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0 LIMIT 1");
        if (!empty($prev_gid))
        {
        ...
        $next_gid = $db->getOne("SELECT max(goods_id) FROM " . $hhs->table('goods') . " WHERE cat_id=".$goods['cat_id']." AND goods_id < ".$goods['goods_id'] . " AND is_on_sale = 1 AND is_alone_sale = 1 AND is_delete = 0");
        if (!empty($next_gid))
        {
...
$total = get_buy_sum($goods_id)+$sales_num;
...
$smarty->assign('desc', "已有{$total}人购买，快来一窝蜂！" );
/*
