...
if ($act == 'default'){
	...
	if(($team_info['pay_time']+24*3600<gmtime())&&$team_info['team_status']==1){
   ...
   //'http://'.$_SERVER['HTTP_HOST']."/".$goods_list[0]['goods_thumb']
    $smarty->assign('title',$goods_list[0]['goods_name'].($d_num<1?"（本团已满）":""));
    $smarty->assign('desc',  $d_num<1?("^_^不好意思，没位置了，您可以再开一团"):("我已参团，还差{$d_num}人，快来一窝蜂！"));//
