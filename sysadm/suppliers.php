...
/* ------------------------------------------------------ */
...

    {
        ...
        echo hhs_iconv(EC_CHARSET, 'GB2312', '推荐人') . "\t";
        
        echo hhs_iconv(EC_CHARSET, 'GB2312', 'QQ号') . "\t";
        
        echo hhs_iconv(EC_CHARSET, 'GB2312', '电话') . "\t";
        
        echo hhs_iconv(EC_CHARSET, 'GB2312', 'email') . "\t";
...

        {
            ...
            echo hhs_iconv(EC_CHARSET, 'GB2312', $value['recommend_person']) . "\t";
            
            echo hhs_iconv(EC_CHARSET, 'GB2312', $value['qq']) . "\t";
            
            echo hhs_iconv(EC_CHARSET, 'GB2312', $value['phone']) . "\t";
            
            echo hhs_iconv(EC_CHARSET, 'GB2312', $value['email']) . "\t";
...
if ($_REQUEST['act'] == 'accounts_query') {
    ...
    check_authz_json('suppliers_accounts_manage');
...

{
...

    {
...

        {
            ...
            $commission = $db->getOne("select sum(commission*goods_number) as total from " . $hhs->table('order_goods') . " where order_id='$value[order_id]'");
            
            $db->query("insert into " . $hhs->table('suppliers_accounts_detal') . " (order_sn,order_id,order_time,commission,suppliers_accounts_id,amount) values ('$value[order_sn]','$value[order_id]','$value[pay_time]','$commission','$suppliers_accounts_id','$value[total_fee]')");
...
elseif (in_array($_REQUEST['act'], array('add','edit'))) 
{
...

    {
        ...
        $smarty->assign('cityes', get_regions(2, $suppliers['province_id']));
